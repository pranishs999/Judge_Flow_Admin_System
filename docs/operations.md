...
- **Resolution:**
  ...
  2. Execute query checking existing scores for the pair:
     ...
     ```
  3. If missing a middle index, the administrator can insert a temporary score or instruct the judge to refresh the mobile cache (syncing the local IndexedDB state).
...
- **DB Settings Tuning:**
  ...
  ALTER SYSTEM SET shared_buffers = '256MB'; -- Tuned based on hosting tier resources
  ```
