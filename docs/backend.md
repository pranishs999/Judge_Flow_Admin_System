...
- **Role:** Retrieves registration data, generates QR images using `deno-qrcode`, compiles them into an A4 PDF using `pdfkit`, and saves the file to the `qr-codes` private bucket.
...
supabase sso create # configuration script for storage rules
