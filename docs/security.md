...
CREATE POLICY "SA can void scores" ON scores
  ...
  WITH CHECK (
    ...
    AND criterion_id = OLD.criterion_id
...
3. Subsequent operations (saving drafts, uploading files, retrieving responses) require passing the `draft_id` as an authorization token.
