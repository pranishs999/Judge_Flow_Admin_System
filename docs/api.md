...
- **Auth:** Authenticated (Admin or Super Admin. Admin can unilaterally open registration or activate judging phase)
- **Request Body:**
  ```json
  {
    "event_id": "834c8928-86d7-4632-a521-789a2444c120",
...
### `compute_rankings`
Executed automatically by the backend system when the event status transitions to `RESULTS_PROCESSING`. The execution is restricted to authorized roles (Admin/SA) and database triggers.
...
Returns active scoring progression metrics for an authenticated judge in an event. The function checks that the caller matches `p_judge_id` or holds Admin/SA privileges.
