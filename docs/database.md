...
CREATE POLICY "profiles_update_self" ON profiles FOR UPDATE
  USING (auth.uid() = id)
  WITH CHECK (auth.uid() = id AND role = (SELECT role FROM profiles WHERE id = auth.uid()) AND status = (SELECT status FROM profiles WHERE id = auth.uid()));
...
-- Public can read/update own drafts via draft_id (handled via RPC or matching header)
...
-- Insert via trigger only (service role, bypasses RLS)
...
-- Readable by anyone (public registration needs access)
...
-- Managed by Admin/SA (subject to form lock trigger)
...
-- Readable by Admin/SA, or event-assigned judges, or public
...
-- Readable by Admins/SA, or public matching draft_id header
...
-- Insert/Update by public matching draft_id, or Admins/SA
...
-- Managed by Admins/SA, or public matching draft_id header
...
-- Only backend service role (bypassing RLS) inserts/updates rankings
```
...
BEGIN
  -- Check permissions
  IF auth.uid() IS NULL OR NOT EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('SUPER_ADMIN', 'ADMIN')) THEN
...
BEGIN
  -- Check permissions: caller must be p_judge_id or Admin/Super Admin
  IF auth.uid() IS NULL OR (auth.uid() != p_judge_id AND NOT EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('SUPER_ADMIN', 'ADMIN'))) THEN
    RAISE EXCEPTION 'Unauthorized';
  END IF;
