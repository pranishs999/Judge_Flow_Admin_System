...
| Rate Limiting | Upstash Redis | API rate limiting in Edge Middleware |
| Monitoring & Tracking | Axiom / Datadog / Sentry | Logging and runtime crash tracking |
...
- Next.js middleware resolves user role from `session.user.app_metadata.role` — injected into the JWT by the `public.custom_access_token_hook` PostgreSQL function. **Do NOT query the `profiles` table for role resolution in middleware.**
...
| `avatars` | Public read; owner write | User profile images |
...
| Variable | Target Console | Description |
|---|---|---|
| `NEXT_PUBLIC_SUPABASE_URL` | Vercel | Supabase project URL |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | Vercel | Supabase anonymous key (public) |
| `SUPABASE_SERVICE_ROLE_KEY` | Vercel | Supabase service role key (server only) |
| `GOOGLE_CLIENT_ID` | Supabase Auth | Google OAuth client ID (set in Supabase Auth console) |
| `GOOGLE_CLIENT_SECRET` | Supabase Auth | Google OAuth client secret (set in Supabase Auth console) |
| `NEXT_PUBLIC_APP_URL` | Vercel | Application base URL |
| `UPSTASH_REDIS_REST_URL` | Vercel | Upstash Redis REST URL for rate limiting middleware |
| `UPSTASH_REDIS_REST_TOKEN` | Vercel | Upstash Redis REST token for rate limiting middleware |
| `SENTRY_DSN` | Vercel | Sentry DSN for server-side error tracking |
| `AI_API_KEY` | Vercel | Optional AI provider key |
| `AI_ENABLED` | Vercel | Feature flag for AI features (defaults to `false` if absent) |
